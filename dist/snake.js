...
/***/ function(module, exports, __webpack_require__) {
	...
	var _lazy = __webpack_require__(9);
	...
	SnakePit.game = function () {
		...
		function checkSelfCollision(snake) {
			...
			// });
			gameRunning = (0, _lazy2.default)(snake.segments).intersection(snake.head).length > 1 ? false : true;
			// return collision;
		...
		function checkFoodCollision(snake, food) {
			// let head = snake.segments[0];
			return (0, _lazy2.default)(snake.segments).contains(food.coordinates);
		...
		function draw() {
			...
			(0, _lazy2.default)(snake1.segments).each(function (segment, index) {
	...
	SnakePit.snake = function () {
		...
		this.init = function () {
			let snakeBody = _lazy2.default.range(snake.length).toArray().map(function (segment, index) {
				return {
